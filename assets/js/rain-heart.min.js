var supportsES6 = (function () {
    try {
      return Function("(a = 0) => a"), !0;
    } catch (_) {
      return !1;
    }
  })(),
  Let_it_fall = (function (_, $) {
    "use strict";
    var l;
    if (!supportsES6) return !1;
    let e = "field",
      t = new Date().getMonth(),
      a = new Date().getDate(),
      c = {
    ...
    if (!n) return;
    let { name: r, animation: s, style: h, active: m, sizes: v, path: o } = n,
      d = (_) => Math.max(..._),
      z = v.reduce((_, $) => _ + $, 0),
      f = (l) => {
        let e = Number(
          _.getComputedStyle($.body)
            .getPropertyValue("font-size")
            .match(/\d+/)[0]
        );
        return parseInt(_.innerWidth / e);
      },
      L = (_) => {
        let $ = parseInt(_ / z),
          l = parseInt(_ % z),
      ...
      p = (_) => {
        ...
        for (; l < _; )
          for (let a of v)
            if (
              (($ += `<svg class="${e}_flake-${r}" style="width:${a}rem; height:${a}rem; fill-rule:evenodd; animation-name:${s}; ${h}" viewBox="0 0 96 96">${o[t]}</svg>`),
              t < o.length - 1 ? t++ : (t = 0),
              ++l >= _)
            )
              break;
      ...
      u = (_) => {
        let l = $.getElementById(e) || $.createElement("div");
        (l.id = e), (l.className = `${e}-${r}`);
        let t = d(v);
        l.setAttribute(
          "style",
          `transform: translate3d(0, -${Math.sqrt(2 * t * t)}rem, 0)`
        ),
          (l.innerHTML = p(_)),
          $.body.insertBefore(l, $.body.childNodes[0]);
      },
      g = (_) => {
        let $ = f(),
          l = L($);
    ...
    _.addEventListener("load", g), _.addEventListener("resize", g);
  })(window, document);
